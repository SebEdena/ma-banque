...
- **Vertical-slice proof command**: the one real end-to-end command built in this spec should be the data-folder-location setting (get/set) — it's genuinely part of "app setup" rather than business logic, and it doubles as the first building block of the future Settings screen (business requirements §4.6) without pulling in the rest of that screen's scope.
- **Data folder & location pointer** (business requirements §2.3.1): the configurable location is a folder (`ma-banque.sqlite` + backups), not a bare file path. The chosen folder path is tracked in a small config file kept outside the SQLite database itself (e.g. JSON in the OS-standard app config directory) — the database can't tell the app where it is, so this pointer must be readable before any connection is opened. Default folder: an app-managed `saves/` subfolder under the OS-standard app data directory.
- **First launch / unreachable folder**: if no pointer exists, or the pointer's folder can't be reached, the app does not silently create or recreate a database — it surfaces the choice (default location vs. user-picked folder) up through a Tauri command for the frontend to prompt on, rather than deciding unilaterally in the backend.
- **Settings folder actions**: "Move data folder" (relocate current folder + backups: copy, verify, then delete old contents; rejected if destination already holds a valid save) and "Open a different folder" (point at another folder; empty/non-matching folders are treated as fresh — create + migrate; a folder with an invalid `ma-banque.sqlite` is rejected) are two distinct operations, not one ambiguous one.
- **Migration backups & downgrade guard**: before running any pending migration on startup, write a timestamped backup of the database into the same folder (e.g. `ma-banque.sqlite.bak-<timestamp>`); keep only the last 3, oldest dropped first. If the opened database's schema is newer than the app's known migrations, refuse to open it and surface an explicit, distinct error — never attempt a migration in that direction.
