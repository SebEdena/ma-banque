...
Complements [00-business-requirements.md](./00-business-requirements.md) and [technical-architecture.md](../architecture/technical-architecture.md). Builds on [01-setup-backend.md](./01-setup-backend.md) and [02-setup-frontend-ci.md](./02-setup-frontend-ci.md), and shares the Settings screen shell with [05-settings-remainder.md](./05-settings-remainder.md) (see note below) — can otherwise be built in parallel with [03-accounts.md](./03-accounts.md).
...
- **Sequencing with `05-settings-remainder.md`**: this spec's UI is a tab inside the Settings screen shell (left sub-nav: Postes / Affichage / Stockage) that `05-settings-remainder.md` builds. Whichever of the two lands first should build the shared Settings shell/sub-nav component; the other fills in its tab's content against it. This is a real (if narrow) UI dependency between the two specs, unlike the picker-sharing arrangement with `03-accounts.md` — flagging it explicitly since the header of both specs otherwise describes all three layer-1 specs as fully parallel.
- The `categories` route/component scaffolded in `02-setup-frontend-ci.md` has no remaining job once this spec lands — whichever of this spec or `05-settings-remainder.md` lands second should remove it (or repoint it to redirect into Settings' "Postes" tab, implementer's call) rather than leave a dead, unreachable placeholder route.
