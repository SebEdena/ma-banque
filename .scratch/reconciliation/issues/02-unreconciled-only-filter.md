...
- [x] `unreconciled_only: bool` added to `EntryListQuery` (Rust) and threaded through `list_entries`/`list_by_account`'s `WHERE` clause.
- [x] The system entry is exempt from the filter (always included), consistent with its existing exemption from the date-range filter.
- [x] `offset_for_date` applies the same predicate as `list_by_account` when the flag is set, so jump-to-date lands on the correct row under the active filter.
...
- [x] SQLite integration test: `offset_for_date` applies the same predicate, landing on the right row with the filter on.
