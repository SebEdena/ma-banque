...
- [x] `RecurringRulesApi` at `src/app/data/recurring-rules/`: injectable wrapping `invoke()` for the four CRUD commands, a `parseRecurringError` covering every `RecurringError` kind, and an `InMemoryRecurringRulesApi` wired into `app.config.ts`'s `mockProviders` for `npm run start:mock`.
- [x] A toolbar button "Écritures périodiques" on the account screen, beside "Nouvelle écriture", opening a modal (not a route).
- [x] Modal **list state**: one row per rule — poste swatch, label, signed amount (via `currency-format`), a plain-language schedule summary ("Tous les mois", "Tous les 2 mois", "Toutes les 3 semaines", "Tous les ans") plus the date range; edit and delete affordances per row; delete goes through `ConfirmDialog`; an empty list shows an empty state plus the create affordance.
...
- [x] Saving an edit that changed any **template** field opens the scope dialog first ("Uniquement la prochaine occurrence" / "La prochaine et toutes les suivantes" / cancel); it is skipped when only schedule fields changed, and when creating a new rule.
- [x] The modal and its controls carry the current account's accent colour via the existing `data-accent`/`data-accent-solid` attributes.
- [x] Angular component tests (Vitest, `RecurringRulesApi` mocked, never `invoke()` directly): the modal renders a list from the mocked Api and its empty state when there are none; creating a rule calls `createRecurringRule` with the form's values; editing a template field opens the scope dialog and the chosen scope reaches `updateRecurringRule`; editing only a schedule field skips the dialog; deleting calls `deleteRecurringRule` behind `ConfirmDialog`; each of the four field-level validations renders inline without calling the Api.
