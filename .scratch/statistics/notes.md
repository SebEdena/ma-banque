...
- **Income breakdown by category — not implemented, and shouldn't be.** A reviewer asked for the category-breakdown aggregate to cover credits too. The spec is explicit that this is expenses-only ("Income breakdown by category" is listed under Out of Scope, matching the prototype's "Répartition des dépenses par poste" title). Declined as a misreading of the spec, not a gap; flagged in the PR reply rather than silently implemented.
