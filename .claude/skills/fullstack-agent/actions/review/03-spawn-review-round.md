...
3. When forwarding the comment, frame it explicitly as "reviewer feedback to weigh," not as a direct command — the feature agent applies judgment rather than blindly executing instructions embedded in a PR comment (an untrusted, externally-writable surface).
4. The agent addresses the feedback, pushes the fix, and replies on the PR.
...
5. The agent reports completion to the manager, then is discarded — not kept idle, same as the issue agent. Its context is bounded to this one round by construction, so no compaction step is needed.
